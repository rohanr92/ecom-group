    ...
    >
      <Link href={href} style={{ display: 'block', textDecoration: 'none' }}>
        <div style={{ position: 'relative', overflow: 'hidden', background: '#f9f9f9', aspectRatio: '3/4' }}>
          ...
          {product.soldOut ? (
            <span style={{ position: 'absolute', top: '10px', left: '10px', background: '#6b6b6b', color: '#fff', fontSize: '9px', fontWeight: 600, letterSpacing: '0.15em', textTransform: 'uppercase', padding: '4px 8px' }}>
              Sold Out
            </span>
          ) : product.badge && (
      ...
      <div style={{ height: '60px', display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', gap: '2px', overflow: 'hidden', marginTop: '6px' }}>
        <Link href={href} style={{ textDecoration: 'none', color: 'inherit' }}>
          ...
          <div style={{ display: 'flex', gap: '6px', alignItems: 'baseline', marginTop: '2px' }}>
            ...
            {onSale && <span style={{ fontSize: '11px', color: '#c0392b', textDecoration: 'line-through' }}>{convert(Number(product.comparePrice))}</span>}
          </div>
        </Link>
      </div>
    </div>
    ...
    >
      ...
      <div style={{ height: '52px', display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', gap: '2px', overflow: 'hidden', marginTop: '6px' }}>
        <Link href={href} style={{ textDecoration: 'none', color: 'inherit' }}>
          ...
          <div style={{ fontSize: '12px', marginTop: '2px', display: 'flex', gap: '6px', alignItems: 'center' }}>
            <span style={{ color: onSale ? '#c0392b' : '#555', fontWeight: onSale ? 600 : 400 }}>{convert(Number(product.price))}</span>
            {onSale && <span style={{ color: '#c0392b', textDecoration: 'line-through', fontSize: '11px' }}>{convert(Number(product.comparePrice))}</span>}
          </div>
        </Link>
      </div>
    </div>
...
function useSlotProducts(slot: string) {
  ...
  useEffect(() => {
    fetch(`/api/collections/slot?slot=${slot}`)
      .then(r => r.json())
      .then(d => setProducts((d.products ?? []).map((p: any) => ({ ...p, soldOut: Array.isArray(p.variants) && p.variants.length > 0 && p.variants.every((v: any) => (v.inventory ?? 0) <= 0) }))))
      .catch(() => {})
  }, [slot])
  return products
}
