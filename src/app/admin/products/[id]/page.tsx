// Save as: src/app/admin/products/[id]/page.tsx
'use client'
...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [newVar,       setNewVar]       = useState<Variant>({
    size: 'S', color: 'Onyx', colorHex: '#1a1a1a', sku: '', inventory: 0,
  })
  ...
  useEffect(() => {
    fetch(`/api/admin/products?id=${id}`)
      ...
      .then(d => {
        ...
        setCategory(p.category     ?? 'Dresses')
        setBadge(p.badge           ?? '')
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!prodRes.ok) { alert(prodData.error ?? 'Failed to save'); setSaving(false); return }
  ...
  if (loading) return (
    <div className="flex items-center justify-center min-h-96">
      <div className="animate-pulse text-gray-400 text-[13px] tracking-wide">Loading product...</div>
    </div>
  )
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[13px] text-[#1a1a1a] font-medium truncate max-w-[200px]">{name}</span>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={deleteProduct}
            className="flex items-center gap-1.5 px-3 py-2.5 border border-red-200 text-red-600 text-[12px] rounded-lg cursor-pointer hover:bg-red-50 bg-white transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                  ...
                  <div className="overflow-x-auto mb-4">
                    <table className="w-full text-left">
                          ...
                          <th className="pb-2 pr-3 w-8">
                            <input type="checkbox"
                              ...
                              onChange={() => setSelectedVars(
                                selectedVars.size === variants.length
                                  ? new Set()
                      ...
                      <tbody className="divide-y divide-gray-50">
                        {variants.map((v, i) => (
                          <tr key={v.id ?? i} className={selectedVars.has(i) ? 'bg-blue-50/30' : ''}>
                            ...
                            <td className="py-2.5 pr-4">
                              <div className="flex items-center gap-1">
                                <button
                                  onClick={() => updateVariant(i, 'inventory', Math.max(0, v.inventory - 1))}
                                  className="w-6 h-6 border border-gray-200 rounded flex items-center justify-center bg-white cursor-pointer hover:bg-gray-100 text-gray-600">
                                  −
                                </button>
                                ...
                                <button
                                  onClick={() => updateVariant(i, 'inventory', v.inventory + 1)}
                                  className="w-6 h-6 border border-gray-200 rounded flex items-center justify-center bg-white cursor-pointer hover:bg-gray-100 text-gray-600">
                                  +
                                </button>
                              </div>
                  ...
                  <div className="border border-dashed border-gray-300 rounded-xl p-4 space-y-3">
                    ...
                    <div className="flex gap-2 pt-1">
                      <button onClick={() => setNewVarOpen(false)}
                        className="px-4 py-2 text-[12px] border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 bg-white text-gray-600">
                        Cancel
                      </button>
                      <button onClick={addVariant}
                        className="px-4 py-2 text-[12px] bg-[#1a1a1a] text-white rounded-lg border-none cursor-pointer hover:bg-gray-800">
                        Add Variant
                      </button>
                    </div>
                  </div>
          ...
          <Section title="Search Engine & URL">
            <div>
              ...
              <div className="flex items-center">
                <span className="px-3 py-2.5 bg-gray-50 border border-r-0 border-gray-300 rounded-l-lg text-[12px] text-gray-400">
                  /products/
                </span>
              ...
              <p className="text-[11px] text-gray-400 mt-1.5">Changing this may break existing links to this product.</p>
            </div>
          </Section>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-red-100 p-5">
            ...
            <button onClick={deleteProduct}
              className="w-full py-2.5 border border-red-200 text-red-600 text-[12px] font-medium rounded-lg cursor-pointer hover:bg-red-50 bg-white transition-colors flex items-center justify-center gap-2">
      ...
      <div className="mt-6 flex items-center justify-between py-4 border-t border-gray-200">
        <Link href="/admin/products"
          className="text-[13px] text-gray-500 hover:text-gray-900 no-underline">
          ← Back to products
        </Link>
        <button onClick={save} disabled={saving}
