...
const FALLBACK_WOMEN = ['New', 'Best Sellers', 'Clothing', 'Dresses', 'Tops', 'Jeans', 'Accessories', 'Sale', 'Clearance']
const FALLBACK_MEN   = ['New', 'Best Sellers', 'Clothing', 'Tops', 'Bottoms', 'Outerwear', 'Accessories', 'Sale', 'Clearance']
...
export default function Navbar() {
  ...
  const [promo, setPromo] = useState({
    text: 'Summer is here!',
    middle: 'FREE SHIPPING ALL OVER THE USA \u2022 CLEARANCE SALE UP TO 70% OFF',
...
const [itemOffset, setItemOffset] = useState(0)
  ...
  useEffect(() => {
    fetch('/api/cms/settings')
      .then(r => r.json())
      .then(d => { if (d.settings?.promo_bar) setPromo(prev => ({ ...prev, ...d.settings.promo_bar })) })
      .catch(() => {})
  }, [])
  ...
  const getNavHref = (label: string) => {
    ...
    if (label === 'Clearance') return '/collections/clearance'
    if (label === 'Clearance') return '/collections/clearance'
...
const openMenu = (item: string) => {
  ...
  return (
    <>
      {promo.active && (<>
      {/* BAR 1: Promo (desktop) */}
      <div className="promo-desktop" style={{ background: '#e8f0e8', borderBottom: '1px solid #d4e0d4', padding: '10px 0' }}>
        <div className="max-container" style={{ padding: '0 clamp(16px,3vw,40px)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '16px' }}>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(13px,1.5vw,18px)', fontStyle: 'italic', color: '#2a4a2a', whiteSpace: 'nowrap', flexShrink: 0 }} className="promo-left">{promo.text}</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', justifyContent: 'center', flex: 1 }}>
            <span style={{ fontFamily: 'var(--font-body)', fontSize: 'clamp(12px,1.8vw,20px)', fontWeight: 700, color: '#1a1a1a', letterSpacing: '0.04em', textTransform: 'uppercase', textAlign: 'center' }}>{promo.middle}</span>
          </div>
          <Link href="/collections/new-arrivals" className="promo-btn" style={{ whiteSpace: 'nowrap', flexShrink: 0 }}>SHOP NEW ARRIVALS</Link>
        </div>
      </div>

      {/* BAR 1b: Promo marquee (mobile only) */}
      {isMobile && (() => {
        // Repeat the admin message as ONE plain string with a single consistent separator.
        // No extra dots, no flex gaps -> every space is identical.
        const one = promo.middle + '\u00A0\u00A0\u00A0•\u00A0\u00A0\u00A0'
        const text = one.repeat(6)
        return (
        <div style={{ background: '#111', overflow: 'hidden', padding: '9px 0', width: '100%' }}>
          <div style={{ display: 'flex', whiteSpace: 'nowrap', width: 'max-content', animation: 'promoScroll 60s linear infinite' }}>
            <span style={{ fontFamily: 'var(--font-body)', fontSize: '12px', fontWeight: 700, letterSpacing: '0.08em', color: '#fff', textTransform: 'uppercase', flexShrink: 0 }}>{text}</span>
            <span style={{ fontFamily: 'var(--font-body)', fontSize: '12px', fontWeight: 700, letterSpacing: '0.08em', color: '#fff', textTransform: 'uppercase', flexShrink: 0 }}>{text}</span>
          </div>
          <style>{`@keyframes promoScroll { from { transform: translateX(0); } to { transform: translateX(-50%); } }`}</style>
        </div>
        )
      })()}
...
<div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: '#fff', overflowY: 'auto' }}>
          <div style={{ padding: '20px 24px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <img src="https://d3o8u8o2i2q94t.cloudfront.net/products/1776112843934-logo-01.png" alt="Solomon & Sage" style={{ height: '32px', width: 'auto' }} />
 ...
 .promo-marquee { display: none; }
 @media (max-width: 768px) {
  .promo-desktop { display: none !important; }
  .promo-marquee { display: block !important; }
