    ...
    <div style={{ maxWidth: '720px' }}>
      ...
      {OPENINGS.map((job, i, arr) => (
        <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 0', borderBottom: '1px solid #f0ece6', gap: '16px', flexWrap: 'wrap' }}>
          ...
          <a href={`mailto:careers@solomonlawrence.com?subject=Application: ${job.title}`} className="btn-outline" style={{ fontSize: '10px', padding: '8px 18px', flexShrink: 0 }}>Apply</a>
        </div>
      ))}
      <div style={{ marginTop: '40px', paddingTop: '32px', borderTop: '1px solid #e8e4de' }}>
        <p style={{ fontFamily: 'var(--font-body)', fontSize: '13px', color: 'var(--color-mid)', lineHeight: 1.8 }}>
          Don't see your role? Send your resume to <a href="mailto:careers@solomonlawrence.com" style={{ color: 'var(--color-charcoal)', textDecoration: 'underline' }}>careers@solomonlawrence.com</a>
        </p>
      </div>
    </div>
  )
